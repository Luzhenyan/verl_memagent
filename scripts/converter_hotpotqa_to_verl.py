...
def convert_hotpotqa_sample(sample: Dict[str, Any], doc_index: int, output_dir: Path) -> Dict[str, Any]:
    ...
    # 创建空的summary文件
    summary_file_path = Path("/usr/wangyicheng/data/hotpotqa_eval/summary") / f"document_{doc_index}_summary.txt"
...
def convert_hotpotqa_dataset(input_file: Path, output_file: Path, output_dir: Path):
    ...
    try:
        ...
        # 直接保存为parquet，不转换为JSON字符串（保持字典和列表格式）
        df.to_parquet(output_file, index=False)
        print(f"✓ 成功保存 {len(verl_samples)} 个样本")
        
    except ImportError as e:
        print(f"警告: 无法导入pandas，跳过Parquet保存: {e}")
        print("仅保存了JSON格式")
...
def main():
    ...
    parser.add_argument(
        "--input",
        type=str,
        default="/home/wangyicheng/data/hotpotqa_eval/eval_100.json",
        help="输入JSON文件路径"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="/home/wangyicheng/data/hotpotqa_eval/eval_100/eval_verl.parquet",
        help="输出Parquet文件路径"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/home/wangyicheng/data/hotpotqa_eval/eval_100",
        help="输出目录（用于存储文档文件）"
    )
