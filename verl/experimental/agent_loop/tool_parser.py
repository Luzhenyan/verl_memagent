...
class HermesToolParser(ToolParser):
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        for match in matches:
            try:
                # 清理匹配内容中的异常部分
                clean_match = self._clean_tool_call_content(match)
                function_call = json.loads(clean_match)
    ...
    def _clean_tool_call_content(self, content: str) -> str:
        """清理工具调用内容中的异常部分，并分离多个工具调用"""
        import re
        import json
        
        # 移除可能混入的异常内容
        content = re.sub(r'PrototypeOf:\s*\{\}', '', content)
        content = re.sub(r'instanceof:\s*Object', '', content)
        content = re.sub(r'NRL:\s*\d+', '', content)
        content = re.sub(r'\(lines \d+-\d+\)\s*\{[^}]*passage[^}]*\}', '', content)
