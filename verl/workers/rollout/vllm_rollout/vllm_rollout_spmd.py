...
class vLLMAsyncRollout:
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        print(f"=== DEBUG: vLLMAsyncRollout.__init__ started ===")
        ...
        print("=== DEBUG: About to initialize ZeroMQ ===")
        self.address = self._init_zeromq()
        print(f"=== DEBUG: ZeroMQ address: {self.address} ===")
        print("=== DEBUG: vLLMAsyncRollout.__init__ completed ===")

    def _init_zeromq(self) -> str:
        print(f"=== DEBUG: _init_zeromq - tensor_parallel_size: {self.config.tensor_model_parallel_size} ===")
        tensor_parallel_size = self.config.tensor_model_parallel_size

        # single node: ipc, multi nodes: tcp
        local_world_size = int(os.environ["RAY_LOCAL_WORLD_SIZE"])
        print(f"=== DEBUG: _init_zeromq - local_world_size: {local_world_size} ===")
        socket_type = "ipc" if tensor_parallel_size <= local_world_size else "tcp"
        print(f"=== DEBUG: _init_zeromq - socket_type: {socket_type} ===")

        # File lock to prevent multiple workers listen to same port
        print(f"=== DEBUG: _init_zeromq - Getting file lock ===")
        with FileLock(f"/tmp/verl_vllm_zmq_{getpass.getuser()}.lock"):
            if socket_type == "ipc":
                pid = os.getpid()
                address = f"ipc:///tmp/verl_vllm_zmq_{pid}_{getpass.getuser()}.ipc"
                print(f"=== DEBUG: _init_zeromq - Using IPC address: {address} ===")
            else:
                ip, port = self._get_free_port()
                address = f"tcp://{ip}:{port}"
                print(f"=== DEBUG: _init_zeromq - Using TCP address: {address} ===")
            
            print("=== DEBUG: _init_zeromq - Creating ZeroMQ context and socket ===")
            context = zmq.asyncio.Context()
            self.socket = context.socket(zmq.REP)
            self.socket.bind(address)
            print(f"=== DEBUG: _init_zeromq - Socket bound to: {address} ===")

        print("=== DEBUG: _init_zeromq - Creating async loop task ===")
        loop = asyncio.get_running_loop()
        self.zmq_loop_task = loop.create_task(self._loop_forever())
        print("=== DEBUG: _init_zeromq - ZeroMQ initialization completed ===")
